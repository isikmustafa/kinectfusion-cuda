...
int main()
{
	...
	downSample <<<dim3(320 / threads.x, 240 / threads.y), threads>>> (depth_pyramid[0], depth_pyramid[1]);
	downSample <<<dim3(160 / threads.x, 120 / threads.y), threads>>> (depth_pyramid[1], depth_pyramid[2]);
	end.record();
	end.synchronize();
